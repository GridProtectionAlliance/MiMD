...
import { LoadingIcon, LoadingScreen, Modal, ToolTip, Warning } from '@gpa-gemstone/react-interactive';
import BaseConfigWindow, { BaseConfigByMeter } from '../Common/BaseConfigWindow';
...
const MeterConfigurationWindow = (props: IProps) => {
    ...
    React.useEffect(() => {
        setConfigurationList((lst) => {
            let filtered = lst.filter(item => addedIndices.findIndex(i => i == item.ID) > -1);
    ...
    function RemoveField(item: PRC002.IConfigField, configID: number) {
        ...
        let index = addedIndices.findIndex(item => item == configID);
        setAddedFields(flds => { let updated = _.cloneDeep(flds); updated[index] = updated[index].filter(f => f.ID != item.ID); return updated; })
    ...
    function AddNew(): void {
        ...
        setConfigurationList((lst) => { let updated = _.cloneDeep(lst); updated.push({ ...newConfiguration, ID: newID }); return updated; })
        setAddedIndices((lst) => {
            let updated = _.cloneDeep(lst); updated.push(newID); return updated
        });
        setNewConfiguration({ ID: -1, MeterId: props.MeterID, Name: 'New Configuration', Pattern: '*.ini' });
        setState('edit');
        setAddedFields((lst) => {
            let updated = _.cloneDeep(lst); updated.push([]); return updated
        });
    }
    ...
    function CompleteUpload(): void {
        ...
        setConfigurationList((lst) => { let updated = _.cloneDeep(lst); updated.push({ Name: uniqName, MeterId: props.MeterID, Pattern: '**/' + fileName, ID: newID }); return updated; })
        setAddedIndices((lst) => {
            let updated = _.cloneDeep(lst); updated.push(newID); return updated
        });
        setState('edit');
        setAddedFields((lst) => {
            let updated = _.cloneDeep(lst); updated.push(selectedFields); return updated
        });
    }
    ...
    function saveField() {
        ...
        if (addedIndices.findIndex(i => i == newEditField.BaseConfigId) > -1) {
            let index = addedIndices.findIndex(i => i == newEditField.BaseConfigId);
            setAddedFields((lst) => {
                let updated = _.cloneDeep(lst);
                let findex = updated[index].findIndex(item => item.ID == newEditField.ID);
    ...
    return (
        <>
            ...
            <Warning Title={'Unable to Change Field'} CallBack={(confirm) => { setShowNoEditNotice(false) }} Show={showNoEditNotice}
